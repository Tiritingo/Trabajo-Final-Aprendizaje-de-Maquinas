...
try:
  logisticRegression_model = joblib.load('/content/drive/MyDrive/trabajo_final/save_models/LogisticRegression_pipeline.pkl')
  linearSVC_model = joblib.load('/content/drive/MyDrive/trabajo_final/save_models/LinearSVC_pipeline.pkl')
  KNeighborsClassifier_model = joblib.load('/content/drive/MyDrive/trabajo_final/save_models/KNeighborsClassifier_pipeline.pkl')
  DecisionTreeClassifier_model = joblib.load('/content/drive/MyDrive/trabajo_final/save_models/DecisionTreeClassifier_pipeline.pkl')
  svm_model = joblib.load('/content/drive/MyDrive/trabajo_final/save_models/SMV_pipeline.pkl')
  votingClassifier_model = joblib.load('/content/drive/MyDrive/trabajo_final/save_models/VotingClassifier_pipeline.pkl')
  randomForestClassifier_model = joblib.load('/content/drive/MyDrive/trabajo_final/save_models/RandomForestClassifier_pipeline.pkl')
  XGBoostClassifier_model = joblib.load('/content/drive/MyDrive/trabajo_final/save_models/XGBoostClassifier_pipeline.pkl')
...
*   The Streamlit application was successfully set up with a title and an introductory message.
*   A file uploader widget was implemented to accept Excel files (`.xlsx`).
*   The uploaded Excel file is read into a pandas DataFrame, and the head of the DataFrame is displayed as a preview.
*   The application iterates through a dictionary of pre-loaded machine learning models and makes predictions on the uploaded data. Error handling is included for models that fail to predict.
*   The predictions from each model are stored in a dictionary, and a comparative DataFrame of these predictions is created and displayed.
*   An optional feature was added to allow users to select specific models for prediction using a multiselect widget. The prediction and display logic updates based on the user's selection.
*   Another optional feature was implemented to calculate and display performance metrics (Accuracy, Precision, Recall, F1 Score) for the models, provided the user specifies the column containing the true labels in the uploaded data. Error handling is included for invalid column names or data mismatches.

### Insights or Next Steps

*   The application successfully integrates file upload, data display, model prediction, and comparative results presentation.
*   Consider adding visualizations (e.g., bar charts comparing model metrics) to enhance the interpretation of model performance.
